...
`RSLBuilder` covers the full BTS→BSC set (DATA_IND/REQ, UNIT_DATA_*, CHAN_ACTIV_ACK/NACK, RF_CHAN_REL_ACK, CONN_FAIL, MEAS_RES, HANDO_DET, CCCH_LOAD_IND, CHAN_RQD, DELETE_IND), each with a `std::vector` and a zero-alloc `span` overload.
