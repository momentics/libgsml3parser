...
- **C89.** The header compiles as strict C89..C17
  (`gcc`/`clang -std=cNN -pedantic-errors -Werror`, MSVC `/TC /W4 /WX`)
  and as C++20; CI enforces this matrix. `tests/c_api_c89_check.c` is the
  reference for consuming the API from plain C.
...
(dllexport/dllimport / default visibility). On MSVC the DLL also
